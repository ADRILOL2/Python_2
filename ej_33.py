...
def trobada(canviTalaiot, puntuación):
    p=puntuación
    ...
    if canviTalaiot == str(gegantamic):
        p+=1
        print ("Et convida a menjar..., tienes {} vidas".format(p))
    else:
        p-=1
        print ("Se't menja d'un mos...ÑAMÑAMÑAM, te quedan {} vidas".format(p))
    return p

# Funció principal 
partidaNova = ("si")
puntuación=1
while puntuación>0 and (partidaNova==("s") or partidaNova==("si")):
    intro()
    if puntuación!=0:
        nTalaiot = canviTalaiot()
        puntuación=trobada(nTalaiot, puntuación)
